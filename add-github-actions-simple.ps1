# Add GitHub Actions IP ranges using broader cloud provider ranges
# This avoids the 4000 source address prefix limit
...
Write-Host "Adding GitHub Actions rules using broader cloud provider ranges..." -ForegroundColor Green
...
# Major cloud provider ranges that GitHub Actions uses
# These are the main ranges where GitHub Actions runners are hosted
$githubActionsRanges = @(
    "13.64.0.0/11",      # Microsoft Azure (East US)
    "13.104.0.0/14",     # Microsoft Azure (East US 2)
    "13.107.0.0/16",     # Microsoft Azure (East US 2)
...
Write-Host "Using $($githubActionsRanges.Count) broader IP ranges" -ForegroundColor Yellow
...
Write-Host "`nNote: Using broader cloud provider ranges to avoid the 4000 prefix limit." -ForegroundColor Yellow
